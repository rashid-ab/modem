...
const BrandsShowroom = props => {
  ...
  const [filteredShowRooms, setFilteredShowRooms] = useState(brandShowroomData.length && brandShowroomData || []);
  ...
  useEffect(() => {
    ...
    brandShowroomData?.length && Object.keys(brandShowroomData[0].indexes).map(key => {
      const arr = [];
  ...
  const renderShowrooms = (filteredShowRooms.length && Object.keys(filteredShowRooms[0]?.indexes).length) ? Object.keys(filteredShowRooms[0].indexes).map((index, key) => <View
    ...
    >
        ...
        {filteredShowRooms[0].indexes[index].map((data, key) => <BrandsShowroomByAlphabet 
          key={key}
          brandShowroomData={data}
        />)}
      </View>
    ...
    {loading ? <View style={styles.centerMe}><ActivityIndicator size="large" color= "black"/></View>
      ...
      <View style={{flex: 1}}>
        ...
        <View style={styles.rootContainer}>
          ...
          <View style={[styles.headingMainContainer, {marginTop: other ? 0 : -1}]}>
          ...
          <ScrollView style={{flex: 1, paddingHorizontal: 8}} contentContainerStyle={{flexGrow: 1}} ref={scrollRef} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} />}>
            {!selectedIndex ? 
            <>
            <Text style={styles.gernalHeading}>{brandShowroomData && brandShowroomData[0]?.title}</Text>
              {renderShowrooms}
            </>
            : 

...
import { fetchSalesCompaignsMultiLabelShowrooms } from '../../redux/actions/agendasActions'
...
const CompaignMultiLabelShowroom = props => {
  const { loading, fetchSalesCompaignsMultiLabelShowrooms, allSaleCompaignMultiLabelShowrooms } = props;
  const [refreshing, setRefreshing] = useState(false);
  const [selectedIndex, setSelectedIndex] = useState(0);
  const [alphaPos, setAlphaPos] = useState({});
  const [lettersViewHeight, setLettersViewHeight] = useState();
  const [filteredShowRooms, setFilteredShowRooms] = useState(allSaleCompaignMultiLabelShowrooms || []);
  const [showDetailViewHeight, setShowDetailViewHeight] = useState();
  const scrollRef = useRef();

  useEffect(() => {
    fetchSalesCompaignsMultiLabelShowrooms();
  }, [])
  ...
  const renderShowrooms = (allSaleCompaignMultiLabelShowrooms?.length && Object.keys(allSaleCompaignMultiLabelShowrooms[0].indexes).length) ? Object.keys(allSaleCompaignMultiLabelShowrooms[0].indexes).map((index, key) => <BrowseBrandsListByAlphabet 
    key={key}
    compaignBrands={allSaleCompaignMultiLabelShowrooms[0].indexes[index]}
    letter={index}
    alphaPos={alphaPos}
    ...
    {loading ? <View style={styles.centerMe}><ActivityIndicator size="large" color= "black"/></View>
      ...
      <View style={{flex: 1}}>
        ...
        <View style={styles.rootContainer}>
          ...
          >
            <Text style={styles.cityName}>Multi-Label Showrooms</Text>
          </View>
          ...
          <ScrollView style={{flex: 1, paddingHorizontal: 8}} contentContainerStyle={{flexGrow: 1}} ref={scrollRef} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} />}>
            {!selectedIndex ? 
            <>
            <Text style={styles.gernalHeading}>{allSaleCompaignMultiLabelShowrooms && allSaleCompaignMultiLabelShowrooms[0]?.title}</Text>
...
CompaignMultiLabelShowroom.propTypes = {
  navigation: PropTypes.shape({
    navigate: PropTypes.func,
  }),
}

CompaignMultiLabelShowroom.defaultProps = {
  navigation: { navigate: () => null },
}

const mapStateToProps = state => {
  return {
    allSaleCompaignMultiLabelShowrooms: state.agendas.saleCompaignMultiLabelShowrooms,
